...
class Strategy:
    ...
    def buy_decision(self, landing_site_value, landing_site_rent) -> bool:
        if self.name=='Impulsive':
            return self.player.balance>=landing_site_value #impulsivo compra sempre que tem dinheiro       
        elif self.name=='Demanding':     
            return (self.player.balance>=landing_site_value) and (landing_site_rent>50) #demanding compra sempre que aluguel >50 e tenha saldo       
        elif self.name=='Cautious':
            return self.player.balance>=(80+landing_site_value) #cauteloso compra sempre que sobre +80 caso compre        
        else:
            return (self.player.balance>=landing_site_value) and (random.randrange(1,3,1)==1) #randomico compra sempre que tenha dinheiro e considerando teste randomico       
    #depois tentar fazer usando subclasses dentro da classe estrategia em vez de usar o trem de ifs

class Player:
    '''È docstring o nome dessa porcaria, da pra usar nas classes tbm'''
    def __init__(self, strategy_name: str, balance: float, lap: int, position: int ) -> None:
        self.balance = balance
        self.lap = lap
        self.position = position
        self.strategy: Strategy = Strategy(strategy_name, self)
    ...
    #outra forma de fazer a mesma coisa mais avançada que seria usado na forma da linha65
    # @classmethod
    # def generate_player(cls, estrategia):
    #     p1 = cls(estrategia, 300, 0, 0)
    #     return p1
    
class PlayerQueue: #corrigir nomes das classes para CamelCase
    #este é um constructor que é um metodo de instancia
    def __init__(self) -> None: #como retornar o valor da função interna?
    ...
    def _generate_queue(self):
        ...
        # for estrategia in ["Cautious","Demanding"]:
        #     player=Player.generate_player(estrategia)
...
veronica=PlayerQueue()
print(veronica.queue)

#print(PlayerQueue().queue) equivale ao codigo da linha de cima, mas como instancia novamente quando printa saem valores diferentes
